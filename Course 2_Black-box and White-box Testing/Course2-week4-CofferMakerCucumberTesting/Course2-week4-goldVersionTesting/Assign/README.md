...
### Deliverables
Your task is to create two files: `CoffeeMaker.feature` and `TestSteps.java`, which together test the CoffeeMaker application to ensure it is working properly.

Inside the project, you will find the functional code, a couple of unit tests to get you started. The goal is to construct a sufficient number of unit tests to find most of the bugs in the "buggy" version of the coffee maker that is included. You should be able to detect at least 5 bugs in the code using your Cucumber tests.

There should be at least one Cucumber test for each use case in the requirements document (`Requirements-CoffeeMaker.pdf`). 
...
### From Command-Line
To compile from command-line, execute `./gradlew build` (on Linux\UNIX; including macOS) or `gradlew.bat build`.  This will download gradle and all required libraries (on the first run only).  Then, it will compile the code, execute the Cucumber tests, and, if all tests pass, measure the code coverage.  The test and coverage results are emitted as reports to:
 ...
 * `src/test/java` -- the test code
   - `edu.ncsu.csc326.coffeemaker.RunCukesTest` -- the JUnit test runner that allow us to execute our Cucumber tests.  Do not modify this file!
   - `edu.ncsu.csc326.coffeemaker.TestSteps` -- contains the test step implementation (the Java code) that translate Gherkin steps into concrete actions.  You need to update this file.
 * `src/test/resources` -- contains the non-code files needed to execute the tests (including your Gherkin-based specification of the system tests).
   - `edu/ncsu/csc326/coffeemaker/CoffeeMaker.feature` -- the Gherkin-based specification of your Coffee Maker tests.  You need to update this file.
